...
- (void)awakeFromNib {
  ...
  [self.studentList
      registerForDraggedTypes:@[(NSString *)kUTTypeVCard,
...
- (BOOL)tableView:(NSTableView *)tv
  ...
  if ([selectedObjects count] != 0) {
    ...
    copyData = [NSArchiver archivedDataWithRootObject:copyDictionary];
  }
    ...
    #pragma mark InsertPerson
        ...
        if (movedPerson) {
          ...
          NSDictionary *insertedPersonsDictionary =
              [NSUnarchiver unarchiveObjectWithData:movedPerson];
...
- (void)tableViewSelectionDidChange:(NSNotification *)aNotification {
  ...
  if ([myNotifTabView isKindOfClass:[DHGRStudentTableView class]] &&
    ...
    NSIndexSet *toBeChanged = [NSUnarchiver
                                 unarchiveObjectWithData:[[[self.students selectedObjects] firstObject] valueForKey:@"currentReportSelection"]];
