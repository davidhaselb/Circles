...
#define YOUR_STORE_TYPE NSBinaryStoreType
...
- (BOOL)importFileAtPath:(NSString *)filePath
  ...
  NSManagedObjectID *oid = [[self persistentStoreCoordinator]
      managedObjectIDForURIRepresentation:objectURI];

  if (!oid) {
    NSLog(@"%@:%@ to find object id from path %@", [self class],
          NSStringFromSelector(_cmd), filePath);
    return NO;
  }

  NSManagedObject *instance = [[self managedObjectContext] objectWithID:oid];
  //
  //    // how you process each instance will depend on the entity that the
  //    instance belongs to
  //
  if ([[[instance entity] name] isEqualToString:@"DHGRStudent"]) {

    //        // set the display name for Spotlight search result
    //
    NSString *yourDisplayString = [NSString
        stringWithFormat:@"Circles %@",
                         [instance valueForKey:@"kMDItemContactKeywords"]];
    spotlightData[(NSString *)kMDItemDisplayName] = yourDisplayString;

    /*
//            Determine how you want to store the instance information in
'spotlightData' dictionary.
//            For each property, pick the key kMDItem... from MDItem.h that best
fits its content.
//            If appropriate, aggregate the values of multiple properties before
setting them in the dictionary.
...
//            To determine if a property should be indexed, call
...
//// Returns the managed object model. The last read model is cached in a global
///variable and reused if the URL and modification date are identical
- (NSManagedObjectModel *)managedObjectModel {
  if (_managedObjectModel != nil)
    return _managedObjectModel;

  NSDictionary *modelFileAttributes = [[NSFileManager defaultManager]
      attributesOfItemAtPath:[self.modelURL path]
                       error:nil];
  NSDate *modelModificationDate = modelFileAttributes[NSFileModificationDate];

  if ([cachedModelURL isEqual:self.modelURL] &&
      [modelModificationDate isEqualToDate:cachedModelModificationDate]) {
  ...
  if (!_managedObjectModel) {
    ...
    if (!_managedObjectModel) {
      NSLog(@"%@:%@ unable to load model at URL %@", [self class],
            NSStringFromSelector(_cmd), self.modelURL);
      return nil;
    }

    // Clear out all custom classes used by the model to avoid having to link
    // them
    // with the importer. Remove this code if you need to access your custom
    // logic.
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
  ...
  if (![_persistentStoreCoordinator addPersistentStoreWithType:YOUR_STORE_TYPE
                                                 configuration:nil
                                                       ...
                                                       options:nil
                                                         error:&error]) {
    NSLog(@"%@:%@ unable to add persistent store coordinator - %@",
          [self class], NSStringFromSelector(_cmd), error);
  }

  return _persistentStoreCoordinator;
}

//// Returns the managed object context for the importer; already bound to the
...
- (NSManagedObjectContext *)managedObjectContext {
  ...
  if (!coordinator) {
    NSLog(@"%@:%@ unable to get persistent store coordinator", [self class],
